...
import sherpa.utils
from sherpa.utils import NoNewAttributesAfterInit, bool_cast_scalar, \
    is_subclass, is_iterable_not_str, export_method, send_to_pager
...
class Session(NoNewAttributesAfterInit):
    ...
    def save(self,
        ...
        if os.path.isfile(filename) and not bool_cast_scalar(clobber):
    ...
    def save_arrays(self,
        ...
        """
        _check_str_type(filename, "filename")
        sherpa.io.write_arrays(filename, args, fields=fields, sep=sep,
                               comment=comment,
                               clobber=bool_cast_scalar(clobber),
    ...
    def save_source(self,
        ...
        """
        self._save_type('source', id, filename,
                        clobber=bool_cast_scalar(clobber), sep=sep,
                        comment=comment, linebreak=linebreak,
                        format=format)
    ...
    def save_model(self,
        ...
        """
        self._save_type('model', id, filename,
                        clobber=bool_cast_scalar(clobber), sep=sep,
                        comment=comment, linebreak=linebreak,
                        format=format)
    ...
    def save_resid(self,
        ...
        """
        self._save_type('resid', id, filename,
                        clobber=bool_cast_scalar(clobber), sep=sep,
                        comment=comment, linebreak=linebreak,
                        format=format)
    ...
    def save_delchi(self,
        ...
        """
        self._save_type('delchi', id, filename,
                        clobber=bool_cast_scalar(clobber), sep=sep,
                        comment=comment, linebreak=linebreak,
                        format=format)
    ...
    def paramprompt(self, val: bool = False) -> None:
        ...
        """
        self._paramprompt = bool_cast_scalar(val)
    ...
    def _get_model_component(self,
        ...
        if bool_cast_scalar(require) and cmpt is None:
    ...
    def _get_plot_objects(self,
        ...
        **kwargs
        ...
        # Each getfunc call returns the plot object for the given plot
        # type. Repeated calls will return the same object, so a copy
        # is needed. However, we want the first identifier to be
        # "remembered" by the system, to retain the behaviour prior to
        # 4.18.0 (and to support the common case of only being sent a
        # single identifier).
        #
        # So, reverse the identifier list to process them in reverse
        # order (so that the first identifier is evaluated last),
        # copying all but the last one. Then reverse this list when
        # adding the plots to the MultiPlot.
        #
        idvals = self._get_idlist(ids)
        nids = len(idvals)
        plots = []
    ...
    #
    # There is currently no support for multiple identifiers as it is
    ...
    # TODO: need to work out how to handle recalc for the get call to
    # allow multiple ids, as well as how to send in multiple arguments
    ...
    def get_int_unc(self,
        ...
        if recalc:
            ...
            ids, fit = self._get_fit(id, otherids)
            plotobj.prepare(min=min, max=max, nloop=nloop, delv=delv,
                            fac=fac, log=bool_cast_scalar(log),
                            numcores=numcores)
    ...
    def image_fit(self,
        ...
        if bool_cast_scalar(deleteframes):
