...
# pyfits.CardList is deprecated (part of #27 fix)
if hasattr(pyfits, 'Header'):
    _new_header = pyfits.Header
else:
    _new_header = pyfits.CardList
...
def _add_keyword(hdrlist, name, val):
    ...
    if name in ['', 'COMMENT', 'HISTORY']:
        ...
        for v in val:
            hdrlist.append(pyfits.Card(name, v))

    else:
        hdrlist.append(pyfits.Card(name, val))
