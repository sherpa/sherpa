...
def test_arf1d_no_pha_no_exposure_basic():
    ...
    adata = ui.create_arf(egrid[:-1], egrid[1:])
    arf = ARF1D(adata)
...
def test_arf1d_no_pha_no_exposure_call(exposure):
    ...
    adata = ui.create_arf(egrid[:-1], egrid[1:], specresp,
                       exposure=exposure)
    arf = ARF1D(adata)
...
def test_rspmodelnopha_matrix_call():
    ...
    adata = ui.create_arf(rdata.energ_lo, rdata.energ_hi,
...
def test_rsp_normf_error(analysis):
    ...
    adata = ui.create_arf(rdata.energ_lo,
                       rdata.energ_hi,
                       specresp,
                       exposure=exposure)
...
def test_arf1d_no_pha_zero_energy_bin():
    ...
    with pytest.raises(DataErr) as exc:
        ui.create_arf(elo, ehi, specresp, exposure=exposure)
...
def test_arf1d_pha_zero_energy_bin():
    ...
    with warnings.catch_warnings(record=True) as ws:
        warnings.simplefilter("always")
        adata = ui.create_arf(elo, ehi, specresp, exposure=exposure1,
                           ethresh=ethresh)
