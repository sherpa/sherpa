...
class Session(sherpa.ui.utils.Session):
    ...
    def _load_data(self, id, datasets):
        ...
        if num > 1:
            msg = f"Multiple data sets have been input: {ids[0]}-{ids[-1]}"
        else:
            msg = f"One data set has been input: {ids[0]}"
...
#             raise ArgumentError('Response IDs must be contained in a list')
    ...
    def set_grouping(self, id, val=None, bkg_id=None):
        ...
        .. versionchanged:: 4.15.1
           The filter is now reported, noting any changes the new
           grouping scheme has made, if the data set is grouped.
    ...
    def set_bkg_model(self, id, model=None, bkg_id=None):
        ...
        self._background_sources.setdefault(idval, {})[bkg_id] = model

        self._runparamprompt(model.pars)

        # Delete any previous model set with set_full_bkg_model()
        bkg_mdl = self._background_models.get(idval, {}).pop(bkg_id, None)
        if bkg_mdl is not None:
            warning(f"Clearing background convolved model\n'{bkg_mdl.name}'\n" +
                    f"for dataset {idval} background {bkg_id}")
    ...
    def _add_extra_data_and_models(self, ids, datasets, models,
        ...
        # out but we still create it.
        #
        out = {} if bkg_ids is None else bkg_ids
    ...
    def _get_stat_info(self):
        ...
        # TODO: document why the stat argument is set to None here.
        f = self._get_fit_obj(datasets, models, None)
        statinfo = f.calc_stat_info()
        if len(ids) == 1:
            statinfo.name = f'Dataset {ids}'  # TODO: should this be ids[0]?
        else:
