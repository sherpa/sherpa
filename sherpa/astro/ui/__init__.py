...
"""Create a session for modeling and fitting Astronomy data.

Provide access to the functionality of the
`sherpa.astro.ui.utils.Session` class, hiding the access to the Sesson
object.
...
`sherpa.models.basic`, `sherpa.models.template`,
`sherpa.astro.models`, `sherpa.astro.optical`, `sherpa.instrument`,
`sherpa.astro.instrument`, and, if XSPEC support is enabled,
`sherpa.astro.xspec`.

What symbols are exported?
==========================

A number of routines from `sherpa.utils` - and `sherpa.astrp.xspec` if
XSPEC support is enabled - and data classes from both `sherpa.data`
and `sherpa.astro.data` are re-exported by this module, but most are
taken from the list generated by
`sherpa.astro.ui.utils.Session._export_names` when applied to the
output of ``globals()``.
