...
import logging
from typing import Any, Literal, TypeVar, overload, Self
import warnings
...
# Subtypes of Data will use matching DataSapce, e.g. Data1D uses DataSpace1D, Data2D uses DataSpace2D, etc.
# However, that means that the return type is different from the superclass, which is hard to type correctly.
# This catch-all Type Annotation can be used instead.
AnyDataSpace = TypeVar('AnyDataSpace', bound=EvaluationSpaceND)
...
def _check(array: ArrayType | None) -> np.ndarray | None:
...
class Data(BaseData):
    ...
    *valid* and ``False`` as *invalid* (note that this is opposite to
    ...
    @dep.setter
    def dep(self, val: ArrayType | None) -> None:
        self.y = val
    ...
    def set_dep(self, val: ArrayType | float) -> None:
        """
    ...
    @property
    def staterror(self) -> np.ndarray | None:
    ...
    @property
    def syserror(self) -> np.ndarray | None:
