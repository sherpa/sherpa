...
class Axis(metaclass=ABCMeta):
    ...
    _is_ascending: Optional[bool] = None
...
class PointAxis(Axis):
    ...
    def __init__(self, x: Optional[ArrayType]) -> None:
...
class IntegratedAxis(Axis):
    ...
    _lo: Optional[np.ndarray] = None
    _hi: Optional[np.ndarray] = None

    def __init__(self,
                 lo: Optional[ArrayType],
                 hi: Optional[ArrayType]
                 ) -> None:
    ...
    @property
    def lo(self) -> Optional[np.ndarray]:
        return self._lo

    @property
    def hi(self) -> Optional[np.ndarray]:
        return self._hi
...
class EvaluationSpace1D:
    ...
    def __init__(self,
                 x: Optional[ArrayType] = None,
                 xhi: Optional[ArrayType] = None
                 ) -> None:
        """The input arrays are used to instantiate a single axis."""

        self.x_axis: Union[PointAxis, IntegratedAxis]
        if xhi is None:
    ...
    #
    @property
    def grid(self) -> Union[tuple[Optional[np.ndarray]],
    ...
    @property
    def midpoint_grid(self) -> Optional[np.ndarray]:
...
class EvaluationSpace2D:
    ...
    @property
    def grid(self) -> Union[tuple[np.ndarray, np.ndarray],
...
class ModelDomainRegridder1D:
    ...
    def __init__(self,
                 evaluation_space: Optional[EvaluationSpace1D] = None,
                 name: str = 'regrid1d',
                 **kwargs) -> None:
        self.name = name
        self.integrate = True
    ...
    @grid.setter
    def grid(self, value: Union[np.ndarray, tuple[np.ndarray, ...]]) -> None:
    ...
    def _make_and_validate_grid(self, args_array)  -> EvaluationSpace1D:
        ...
        if requested_eval_space.is_empty:
            raise DataErr("emptyaxis")  # is this sensible?
...
class ModelDomainRegridder2D:
    ...
    def __init__(self,
                 evaluation_space: Optional[EvaluationSpace2D] = None,
