...
from sherpa.utils.parallel import SupportsProcess, SupportsQueue, \
    multi, ncpus, context, process_tasks
...
#
# ArrayVal is almost sherpa.utils.types.ArrayType, but has
# SupportsFloat rather than float.
#
ArrayVal = Sequence[SupportsFloat] | np.ndarray
...
class FitFunc(Protocol):
    """Represent sherpa.fit.IterFit.fit"""
...
class _EstFunc(Protocol):
    """Represent the function sent to EstMethod."""

    # More arguments could be included but that would require
    # re-working the covariance call not to take a verbose flag (or to
    # move it to the end).
    #
    def __call__(self,
                 pars: np.ndarray,
...
class Callback:
    """Evaluate callback that returns multiple items and return the first.
...
# combine Callback and DropExtraArgs
...
class Confidence(EstMethod):
    ...
    def compute(self,
        ...
        statcb = CallbackDropArgs(statfunc)
...
class ProjWorker:
    ...
    def __init__(self, pars, *, parmins, parmaxes, parhardmins,
                 parhardmaxes, sigma, eps, tol, maxiters, remin,
                 stat_cb, fit_cb, report_progress):
        self.pars = pars
    ...
    def __call__(self, i, singleparnum, lock=None):
...
class ConfidenceWorker:
    ...
    # Is lock ever set? Can it be set in __init__ instead?
    #
    def __call__(self, counter, singleparnum, lock=None):
...
class ParWorker:
    ...
    #
    def __call__(self, parids, parnums):
        results = []
...
def parallel_est(estfunc: Callable,
