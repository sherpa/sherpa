...
from sherpa.utils import NoNewAttributesAfterInit, parallel_map, random
...
def multivariate_t(mean: ArrayType,
    ...
    normal = random.multivariate_normal(rng, np.zeros_like(mean), cov, size=size)
...
class ParameterScaleMatrix(ParameterScale):
    ...
    def get_scales(self,
        ...
        if myscales is None:
            ...
            cov = r.extra_output
            if cov is not None:
                # Scale the covariance matrix (self.sigma is expected
                # to be 1, although it can be changed).
                #
                cov = self.sigma * cov
