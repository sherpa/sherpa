...
/* #include "err.c" */
extern double rescale_error (double err, const double result_abs,
...
int
sao_integration_qng (int (*f)(double *x, int len, void *params),
                     ...
                     double * result, double * abserr, size_t * neval)
{
  ...
  for (k = 0; k < 5; k++)
    {
      ...
      vals1[2*k+1] = center + abscissa;
      vals1[2*k+2] = center - abscissa;

      abscissa = half_length * x2[k];
      vals1[2*k+11] = center + abscissa;
      vals1[2*k+12] = center - abscissa;
    }

  if( EXIT_SUCCESS != f(vals1, 21, params) )
    return -1;
  ...
  for (k = 0; k < 5; k++)
    {
      const double fval1 = vals1[2*k+1];
      const double fval2 = vals1[2*k+2];
      const double fval = fval1 + fval2;
      res10 += w10[k] * fval;
      res21 += w21a[k] * fval;
      resabs += w21a[k] * (fabs (fval1) + fabs (fval2));
  ...
  for (k = 0; k < 5; k++)
    {
      const double fval1 = vals1[2*k+11];
      const double fval2 = vals1[2*k+12];
      const double fval = fval1 + fval2;
      res21 += w21b[k] * fval;
      resabs += w21b[k] * (fabs (fval1) + fabs (fval2));
      savfun[k + 5] = fval;
  ...
  //printf("Compute 43-point formula\n");
  ...
  for (k = 0; k < 11; k++)
    {
      ...
      vals2[2*k]   = center + abscissa;
      vals2[2*k+1] = center - abscissa;
    }  

  if( EXIT_SUCCESS != f(vals2, 22, params) )
    return -1;

  for (k = 0; k < 11; k++)
    {
      const double fval = vals2[2*k] + vals2[2*k+1];
      res43 += fval * w43b[k];
      savfun[k + 10] = fval;
    }
  ...
  //printf("Compute 87-point formula\n");
  ...
  for (k = 0; k < 22; k++)
    {
      ...
      vals3[2*k]   = center + abscissa;
      vals3[2*k+1] = center - abscissa;
    }  

  if( EXIT_SUCCESS != f(vals3, 44, params) )
    return -1;

  for (k = 0; k < 22; k++)
    {
      res87 += w87b[k] * (vals3[2*k] + vals3[2*k+1]);
    }
  ...
  //printf("Gauss-Kronrod Integration Failed!\n");
